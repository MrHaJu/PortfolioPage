...
function typo3extension(userConfig = {}) {
    ...
    return {
        ...
        config(config) {
            ...
            config.resolve ??= {};
            config.resolve.alias = (0, utils_js_1.addAliases)(config.resolve.alias, [
                extension,
            ]);
        ...
        configResolved() {
            if (pluginConfig && pluginConfig.debug) {
                (0, utils_js_1.outputDebugInformation)([extension], entrypoints, pluginConfig.composerContext, logger);
            }
        },
    };
}

...
import { addAliases, addRollupInputs, findEntrypointsInExtensions, initializePluginConfig, outputDebugInformation, } from "./utils.js";
export default function typo3extension(userConfig = {}) {
    ...
    return {
        ...
        config(config) {
            ...
            config.build ??= {};
            config.build.outDir ??= join(pluginConfig.composerContext.path, "Resources/Public/Vite/");
            ...
            entrypoints = findEntrypointsInExtensions([extension], pluginConfig.entrypointFile, pluginConfig.entrypointIgnorePatterns);
        ...
        configResolved() {
            if (pluginConfig && pluginConfig.debug) {
                outputDebugInformation([extension], entrypoints, pluginConfig.composerContext, logger);
            }
        },
    };
}

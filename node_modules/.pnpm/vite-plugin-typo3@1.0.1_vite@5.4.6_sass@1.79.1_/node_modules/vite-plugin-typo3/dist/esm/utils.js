...
export function findEntrypointsInExtensions(extensions, entrypointFile, entrypointIgnorePatterns) {
    ...
    extensions.forEach((extension) => {
        ...
        entrypoints = entrypoints.concat(fg.globSync(patterns, {
            cwd: dirname(file),
...
export function determineRelevantTypo3Extensions(composerContext, entrypointFile) {
    ...
    return installedExtensions.filter((extension) => fs.existsSync(join(extension.path, entrypointFile)));
}
export function outputDebugInformation(relevantExtensions, entrypoints, composerContext, logger) {
    if (relevantExtensions.length) {
        ...
        logger.info(`The following extensions with vite entrypoints have been recognized: ${colors.green(extensionList.join(", "))}`, { timestamp: true });
...
export function addAliases(alias, extensions) {
